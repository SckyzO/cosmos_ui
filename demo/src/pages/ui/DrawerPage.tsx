...
export const DrawerPage = () => {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Drawer / Sheet"
        description="Slide-in panel components for contextual content"
        breadcrumb={
          <PageBreadcrumb
            items={[
              { label: 'Home', href: '/' },
      ...
      {open === 'right' && (
        <>
          ...
          <div className="shadow-theme-xl dark:bg-gray-dark fixed top-0 right-0 z-50 flex h-full w-80 flex-col bg-white">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                This is the right drawer content. Use this for contextual details, filters, or
                secondary actions.
              </p>
            ...
            <div className="border-t border-gray-200 p-4 dark:border-gray-800">
              <button
                ...
                className="bg-brand-500 hover:bg-brand-600 w-full rounded-lg py-2 text-sm font-medium text-white"
              >
                Apply
              </button>
            </div>
      ...
      {open === 'left' && (
        <>
          <Overlay onClick={() => setOpen(null)} />
          <div className="shadow-theme-xl dark:bg-gray-dark fixed top-0 left-0 z-50 flex h-full w-80 flex-col bg-white">
            <div className="flex items-center justify-between border-b border-gray-200 p-5 dark:border-gray-800">
            ...
            <div className="flex-1 p-5">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Left drawer — ideal for navigation menus or secondary sidebars.
              </p>
            </div>
          </div>
        </>
      ...
      {open === 'bottom' && (
        <>
          <Overlay onClick={() => setOpen(null)} />
          <div className="shadow-theme-xl dark:bg-gray-dark fixed right-0 bottom-0 left-0 z-50 rounded-t-2xl bg-white">
            ...
            <div className="p-5">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Bottom sheet — common on mobile for action sheets and pickers.
              </p>
              <div className="mt-4 grid grid-cols-3 gap-3">
                {['Share', 'Copy', 'Edit', 'Delete', 'Archive', 'Export'].map((a) => (
                  <button
                    key={a}
                    onClick={() => setOpen(null)}
                    className="rounded-xl border border-gray-200 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300"
                  >
                    {a}
                  </button>
                ))}
      ...
      {open === 'form' && (
        <>
          <Overlay onClick={() => setOpen(null)} />
          <div className="shadow-theme-xl dark:bg-gray-dark fixed top-0 right-0 z-50 flex h-full w-96 flex-col bg-white">
            <div className="flex items-center justify-between border-b border-gray-200 p-5 dark:border-gray-800">
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <form className="space-y-4">
                {[
                  { l: 'Device Name', p: 'e.g. r01-01-c01' },
                  { l: 'Template ID', p: 'e.g. bs-xh3140-trio-1u' },
                  { l: 'U Position', p: '1' },
                ].map(({ l, p }) => (
                  <div key={l}>
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="e.g. compute[001-004]"
                    className="focus:border-brand-500 w-full rounded-lg border border-gray-200 px-3.5 py-2.5 text-sm focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                  />
                </div>
              </form>
            ...
            <div className="flex gap-3 border-t border-gray-200 p-5 dark:border-gray-800">
              <button
                onClick={() => setOpen(null)}
                className="flex-1 rounded-lg border border-gray-200 py-2 text-sm font-medium text-gray-700 dark:border-gray-700 dark:text-gray-300"
              >
                Cancel
              </button>
              <button
                onClick={() => setOpen(null)}
                className="bg-brand-500 hover:bg-brand-600 flex-1 rounded-lg py-2 text-sm font-medium text-white"
              >
                Save Changes
              </button>
            </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        {[
          {
            type: 'right' as DrawerType,
            label: 'Open Right Drawer',
            desc: 'Slides in from the right edge',
          },
          {
            type: 'left' as DrawerType,
            label: 'Open Left Drawer',
            desc: 'Slides in from the left edge',
          },
          {
            type: 'bottom' as DrawerType,
            label: 'Open Bottom Sheet',
            desc: 'Slides up from the bottom',
          },
          {
