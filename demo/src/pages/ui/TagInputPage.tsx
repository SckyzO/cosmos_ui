...
export const TagInputPage = () => {
  ...
  const mkHandler =
    ...
    (e: KeyboardEvent<HTMLInputElement>) => {
      if (e.key === 'Enter' && input.trim()) {
        e.preventDefault();
        if (!tags.includes(input.trim()) && (!max || tags.length < max))
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Tag Input"
        description="Tag and chip input components"
        breadcrumb={
          <PageBreadcrumb
            items={[
              { label: 'Home', href: '/' },
        ...
        <SectionCard title="Basic Tag Input" desc="Press Enter to add, × to remove">
          ...
          >
            {basic.map((tag, i) => (
              ...
              >
                {tag}
                <button onClick={() => setBasic(basic.filter((_, j) => j !== i))}>
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
            <input
              ...
              onKeyDown={mkHandler(basic, setBasic, input1, setInput1)}
              className="min-w-[100px] flex-1 border-none bg-transparent text-sm outline-none dark:text-white"
              placeholder={basic.length === 0 ? 'Type and press Enter' : ''}
            />
          </div>
        </SectionCard>
          ...
          <div className="relative">
            ...
            >
              {sTags.map((tag, i) => (
                ...
                >
                  {tag}
                  <button onClick={() => setSTags(sTags.filter((_, j) => j !== i))}>
                    <X className="h-3 w-3" />
                  </button>
                </span>
              ))}
              <input
                ...
                onKeyDown={mkHandler(sTags, setSTags, sInput, setSInput)}
            ...
            {suggestions.length > 0 && (
              <div className="shadow-theme-md absolute z-10 mt-1 w-full rounded-lg border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
                {suggestions.map((s) => (
                  <button
                    key={s}
                    onClick={() => {
                    ...
                    className="block w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-white/5"
                  >
                    {s}
                  </button>
                ))}
        ...
        <SectionCard title="Colored Tags" desc="Random color assigned per tag">
          ...
          >
            {colored.map((tag, i) => (
              ...
              >
                {tag.text}
                <button onClick={() => setColored(colored.filter((_, j) => j !== i))}>
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && cInput.trim()) {
                  ...
                  setColored([
                    ...colored,
                    { text: cInput.trim(), color: Math.floor(Math.random() * 4) },
                  ]);
                  setCInput('');
                } else if (e.key === 'Backspace' && !cInput && colored.length > 0)
                  setColored(colored.slice(0, -1));
              }}
              className="min-w-[100px] flex-1 border-none bg-transparent text-sm outline-none dark:text-white"
              placeholder="Add tag"
            />
        ...
        <SectionCard title="Max Tags (5)" desc="Shows limit counter">
          ...
          >
            {maxTags.map((tag, i) => (
              ...
              >
                {tag}
                <button onClick={() => setMaxTags(maxTags.filter((_, j) => j !== i))}>
                  <X className="h-3 w-3" />
                </button>
              </span>
            ))}
            {maxTags.length < MAX && (
              <input
                ...
                onKeyDown={mkHandler(maxTags, setMaxTags, mInput, setMInput, MAX)}
                className="min-w-[100px] flex-1 border-none bg-transparent text-sm outline-none dark:text-white"
                placeholder="Add tag"
              />
            )}
        ...
        <SectionCard title="Disabled Tags" desc="Read-only display">
          <div className="flex min-h-[42px] flex-wrap items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-gray-700 dark:bg-gray-800">
            {['React', 'TypeScript', 'Tailwind CSS', 'Node.js'].map((tag) => (
              <span
                key={tag}
                className="inline-flex items-center rounded-full bg-gray-200 px-2.5 py-1 text-xs font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-400"
              >
                {tag}
              </span>
            ))}
