...
// ── localStorage keys ─────────────────────────────────────────────────────────
// cosmos.theme.mode | cosmos.theme.accent | cosmos.theme.light | cosmos.theme.dark

const LS = {
  mode: 'cosmos.theme.mode',
  accent: 'cosmos.theme.accent',
...
export const ThemeProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [accent, setAccentState] = useState<AccentColor>(() => {
    const saved = localStorage.getItem(LS.accent) as AccentColor | null;
    return saved && VALID_ACCENTS.has(saved) ? saved : 'indigo';
  });

  const [lightTheme, setLightThemeState] = useState<LightTheme>(() => {
    const saved = localStorage.getItem(LS.light) as LightTheme | null;
    return saved && VALID_LIGHT.has(saved) ? saved : 'slate';
  });

  const [darkTheme, setDarkThemeState] = useState<DarkTheme>(() => {
    const saved = localStorage.getItem(LS.dark) as DarkTheme | null;
    return saved && VALID_DARK.has(saved) ? saved : 'void';
  });

  // Sync ThemeContext when AppLayout's header toggle fires via custom event.
  // The toggle lives outside the ThemeProvider subtree, so a custom event is
  // the only way to propagate the change without prop-drilling.
  useEffect(() => {
    const handler = (e: CustomEvent<{ dark: boolean }>) => {
      setModeState(e.detail.dark ? 'dark' : 'light');
    };
    window.addEventListener('cosmos-theme-mode', handler as EventListener);
    return () => window.removeEventListener('cosmos-theme-mode', handler as EventListener);
  }, []);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(LS.light, lightTheme);
    localStorage.setItem(LS.dark, darkTheme);
  }, [mode, accent, lightTheme, darkTheme]);

  // Dispatch 'cosmos-theme-mode' to keep AppLayout's isDark state in sync.
  // setAccent does not dispatch because accent changes do not affect the
  // dark/light toggle — only mode switches need cross-tree coordination.
  const setMode = (m: 'dark' | 'light') => {
    setModeState(m);
    window.dispatchEvent(new CustomEvent('cosmos-theme-mode', { detail: { dark: m === 'dark' } }));
  };
